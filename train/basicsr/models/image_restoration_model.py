...
from basicsr.models.losses import create_loss
...
class ImageRestorationModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        self.all_losses = create_loss(self.opt, rank)
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        out_sides, preds = self.net_g(self.lq)
        if not isinstance(preds, list):
            preds = [preds]
        if not isinstance(out_sides, list):
            out_sides = [out_sides]
        ...
        l3 = 0.
        l4 = 0.
        for out_side, pred in zip(out_sides, preds):
            l1 += self.all_losses['pixel_loss'](pred, self.gt)
            l2 += self.all_losses['perceptual_loss'](pred, self.gt)
            l3 += self.all_losses['edge_loss'](pred, self.gt)
            l4 += self.all_losses['enhance_loss'](out_side, self.gt)

        l_total += l1 + l2 + l3 + l4
        loss_dict['l_pixel_loss'] = l1
        loss_dict['l_perceptual_loss'] = l2
        loss_dict['l_edge_loss'] = l3
        loss_dict['l_enhance_loss'] = l4
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                if j >= n:
                    j = n
                pred = self.net_g(self.lq[i:j], side_loss = False)
