from .losses import MSELoss, L1Loss, CharbonnierLoss, SSIM, VGGLoss, EdgeLoss, FrequencyLoss, EnhanceLoss, LPIPSloss
...
def create_loss(opt, rank):
    ...
    if opt['perceptual']:
        perceptual_loss = LPIPSloss(loss_weight=opt['perceptual_weight']).to(rank)
        losses['perceptual_loss'] = perceptual_loss
        if rank == 0: print(
            f"Using perceptual loss {opt['perceptual_criterion']} with weight {opt['perceptual_weight']}")
    # the edge loss
    if opt['edge']:
